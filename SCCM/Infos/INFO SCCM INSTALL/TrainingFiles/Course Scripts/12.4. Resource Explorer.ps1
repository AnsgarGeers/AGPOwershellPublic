...
#linux machine
Start-Process -Filepath ("$ModulePath") -ArgumentList ('-s -sms:ResExplrQuery="SELECT ResourceID FROM SMS_R_SYSTEM WHERE NAME = ''srv0003.classroom.intranet''" -sms:connection=\\' + $servername + '\root\sms\site_' + $siteCode)
